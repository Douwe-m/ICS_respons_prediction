#What do I need?
  #1) patient data with clinical parameters
  #2) expression data of patients with clinical data
  #3) a signature
...
patient_data <- patient_data %>% 
  select(patientID, treatment, time, gender, agev1, smokingstatusv1, packyearv1, 
         ccqtotalv1, ccqtotalv11, rvtlcpredv1, rvtlcpredv11, fevnaprev1, fevnaprev11) %>% 
  filter(treatment != 0,   #placebo
         treatment != 3,   #ICS + placebo
         time == 0) %>% 
  mutate(d_fev_pred = fevnaprev11 - fevnaprev1,
